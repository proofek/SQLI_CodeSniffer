...
/**
 * ZF_Sniffs_WhiteSpace_FunctionSpacingSniff
 *
 ...
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class ZF_Sniffs_WhiteSpace_FunctionSpacingSniff implements SQLI_CodeSniffer_Sniff
{
    /**
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ((array_key_exists(($closer + 2), $tokens) and ($tokens[($closer + 2)]['content'] === '}')) or
            ...
            if ($tokens[($closer + 2)]['content'] !== '}' ) {
                $phpcsFile->addError("Expected 0 blank lines after last function; $foundLines found", ($closer + 1), 'NoBlankLineAfterLastFunction');
            }
        } else if ($foundLines !== 1) {
            $phpcsFile->addError("Expected 1 blank lines after function; $foundLines found", $closer, 'BlankLineAfterFunction');
        }
        ...
        if ($tokens[$prevContent]['content'] === '{') {
            if ($foundLines !== 0) {
                $phpcsFile->addError("Expected 0 blank lines before function; $foundLines found", $stackPtr, 'BlankLineBeforeFunction');
            }
        } else if ($foundLines !== 1) {
            $phpcsFile->addError("Expected 1 blank line before function; $foundLines found", $stackPtr, 'NoBlankLineBeforeFunction');
        }
    }
}
