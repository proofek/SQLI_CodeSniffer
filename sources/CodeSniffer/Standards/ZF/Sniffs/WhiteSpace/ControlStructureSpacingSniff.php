...
/**
 * ZF_Sniffs_WhiteSpace_ControlStructureSpacingSniff
 *
 ...
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class ZF_Sniffs_WhiteSpace_ControlStructureSpacingSniff implements SQLI_CodeSniffer_Sniff
{
    /**
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$firstContent]['line'] !== ($tokens[$bracket]['line'] + 1)) {
            ...
            $phpcsFile->addError($error, $scopeOpener, 'NoBlankLineStartControlStructure');
        }
        ...
        if ($tokens[$trailingContent]['code'] === T_CLOSE_CURLY_BRACKET) {
            ...
            if ($tokens[$trailingContent]['line'] !== ($tokens[$scopeCloser]['line'] + 1)) {
                ...
                $phpcsFile->addError($error, $scopeCloser, 'BlankLineAfterControlStructure');
            }
        } else {
            if ($tokens[$trailingContent]['line'] === ($tokens[$scopeCloser]['line'] + 1)) {
                $error = 'No blank line found after control structure';
                $phpcsFile->addError($error, $scopeCloser, 'NoBlankLineAfterControlStructure');
            }
        }
    }
}
