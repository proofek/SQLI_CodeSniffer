...
/**
 * ZF_Sniffs_Files_IncludingFileSniff
 *
...
class ZF_Sniffs_Files_IncludingFileSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$nextToken]['code'] === T_OPEN_PARENTHESIS) {
            ...
            $phpcsFile->addError($error, $stackPtr, 'NoParentheseForStatement');
        }
        ...
        if ($tokenCode === T_REQUIRE) {
            ...
            $phpcsFile->addError($error, $stackPtr, 'RequireDepreciated');
        }

        if ($tokenCode === T_INCLUDE) {
            $error = 'The use of "include" is depreciated, use "include_once" instead';
            $phpcsFile->addError($error, $stackPtr, 'IncludeDepreciated');
        }
        ...
        if ($inCondition === true) {
            ...
            if ($tokens[$nextNew]['line'] !== ($tokens[$stackPtr]['line'] + 1)) {
                if ($tokenCode === T_REQUIRE_ONCE) {
                    ...
                    $phpcsFile->addError($error, $stackPtr, 'UseIncludeOnceConditionnalStatement');
                }
            }
        } else {
            ...
            if ($tokenCode === T_INCLUDE_ONCE) {
                ...
                $phpcsFile->addError($error, $stackPtr, 'UseRequireOnceUnconditionnalStatement');
            }
        }
    }
}
