 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class ZF_Sniffs_NamingConventions_ValidVariableNameSniff extends PHP_CodeSniffer_Standards_AbstractVariableSniff
{
    ...
    protected function processVariable(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
            ...
            if ($tokens[$var]['code'] === T_STRING) {
                ...
                if ($tokens[$bracket]['code'] !== T_OPEN_PARENTHESIS) {
                    ...
                    if (PHP_CodeSniffer::isCamelCaps($objVarName, false, true, false) === false) {
                        ...
                        $phpcsFile->addError($error, $var, 'VariableCamelCase');
                    } else if (preg_match('|\d|', $objVarName)) {
                        $warning = "Variable \"$originalVarName\" contains numbers but this is discouraged";
                        $phpcsFile->addWarning($warning, $stackPtr, 'VariableWithoutNumber');
                    }
                }//end if
            }//end if
        }//end if
        ...
        if (PHP_CodeSniffer::isCamelCaps($varName, false, true, false) === false) {
            $error = "Variable \"$originalVarName\" is not in valid camel caps format";
            $phpcsFile->addError($error, $stackPtr, 'VariableCamelCase');
    ...
    protected function processMemberVar(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($public === true) {
            if (substr($varName, 0, 1) === '_') {
                ...
                $phpcsFile->addError($error, $stackPtr, 'NoLeadingUnderscorePublicVariable');
                return;
            }
        } else {
            if (substr($varName, 0, 1) !== '_') {
                ...
                $phpcsFile->addError($error, $stackPtr, 'LeadingUnderscoreScopeVariable');
                return;
            }
        }
