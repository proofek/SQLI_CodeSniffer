...
/**
 * ZF_Sniffs_Functions_FunctionCallSignatureSniff
 *
 ...
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class ZF_Sniffs_Functions_FunctionCallSignatureSniff implements SQLI_CodeSniffer_Sniff
{
    /**
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if (($stackPtr + 1) !== $next) {
            ...
            $phpcsFile->addError($error, $stackPtr, 'NoSpaceBeforeOpeningParenthesisFunctionCall');
        }

        if ($tokens[($next + 1)]['code'] === T_WHITESPACE) {
            // Checking this: $value = my_function([*]...)
            $error = 'Space after opening parenthesis of function call prohibited';
            $phpcsFile->addError($error, $stackPtr, 'NoSpaceAfterOpeningParenthesisFunctionCall');
        }
        ...
        if ($tokens[($closer - 1)]['code'] === T_WHITESPACE) {
            ...
            if ($between !== $closer) {
                ...
                $phpcsFile->addError($error, $closer, 'NoSpaceBeforeClosingParenthesisFunctionCall');
            }
        }
        ...
        if ($tokens[$next]['code'] === T_SEMICOLON) {
            if (in_array($tokens[($closer + 1)]['code'], PHP_CodeSniffer_Tokens::$emptyTokens) === true) {
                ...
                $phpcsFile->addError($error, $closer, 'NoSpaceAfterClosingParenthesisFunctionCall');
            }
        }
    }
}
