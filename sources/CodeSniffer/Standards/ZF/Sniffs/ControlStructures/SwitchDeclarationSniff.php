...
/**
 * ZF_Sniffs_ControlStructures_SwitchDeclarationSniff
 *
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        while ($nextCase !== false) {
            ...
            if ($content !== strtolower($content)) {
                ...
                $phpcsFile->addError($error, $nextCase, 'CaseKeywordLowercase');
            }

            if ($tokens[$nextCase]['column'] !== $caseAlignment) {
                $error = 'CASE keyword must be indented 4 spaces from SWITCH keyword';
                $phpcsFile->addError($error, $nextCase, 'CaseKeywordIndentation');
            }

            if ($tokens[($nextCase + 1)]['type'] !== 'T_WHITESPACE' or $tokens[($nextCase + 1)]['content'] !== ' ') {
                $error = 'CASE keyword must be followed by a single space';
                $phpcsFile->addError($error, $nextCase, 'SpaceAfterCaseKeyword');
            }
            ...
            if ($tokens[($opener - 1)]['type'] === 'T_WHITESPACE') {
                ...
                $phpcsFile->addError($error, $nextCase, 'NoSpaceBeforeColonCaseStatement');
            }
            ...
            if ($nextBreak !== false and isset($tokens[$nextBreak]['scope_condition']) === true) {
                ...
                if ($tokens[$nextBreak]['scope_condition'] === $nextCase) {
                    if ($tokens[$nextBreak]['column'] !== ($caseAlignment + 4)) {
                        ...
                        $phpcsFile->addError($error, $nextBreak, 'BreakStatementIndentation');
                    }
                    ...
                    if ($foundContent === false) {
                        ...
                        $phpcsFile->addError($error, $nextCase, 'NoEmptyCaseStatement');
                    }
                    ...
                    if ($prevLine !== ($breakLine - 1)) {
                        ...
                        $phpcsFile->addError($error, $nextBreak, 'NoBlankLineBeforeBreakStatement');
                    }
                    ...
                    if ($nextLine !== ($breakLine + 2) and $i !== $tokens[$stackPtr]['scope_closer']) {
                        ...
                        $phpcsFile->addError($error, $nextBreak, 'SingleBlankLineAfterBreakStatement');
                    }
                }
            } else {
            ...
            if ($nextLine !== ($caseLine + 1)) {
                ...
                $phpcsFile->addError($error, $nextCase, 'NoBlankLineAfterCaseStatement');
            }
        ...
        if ($default !== false) {
            ...
            if ($nextBreak !== false) {
                ...
                if ($nextLine !== ($breakLine + 1)) {
                    ...
                    $phpcsFile->addError($error, $nextBreak, 'NoBlankLineAfterDefaultsBreakStatement');
                }
            } else {
                $error = 'DEFAULT case must have a BREAK statement';
                $phpcsFile->addError($error, $default, 'DefaultCaseRequiresBreakStatement');
            ...
            if ($foundContent === false) {
                ...
                $phpcsFile->addError($error, $default, 'CommentEmptyDefaultCase');
            }
            ...
            $phpcsFile->addError($error, $stackPtr, 'SwitchStatementMustContainDefaultCase');
        }
        ...
        if ($tokens[$switch['scope_closer']]['column'] !== $switch['column']) {
            ...
            $phpcsFile->addError($error, $switch['scope_closer'], 'ClosingBraceAlignedWithSwitchStatement');
        }

        if ($caseCount === 0) {
            $error = 'SWITCH statements must contain at least one CASE statement';
            $phpcsFile->addError($error, $stackPtr, 'SwitchStatementRequiresCaseStatement');
        }
    }
}
