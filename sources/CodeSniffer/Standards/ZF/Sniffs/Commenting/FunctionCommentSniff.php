...
/**
 * ZF_Sniffs_Commenting_FunctionCommentSniff
 *
...
class ZF_Sniffs_Commenting_FunctionCommentSniff implements SQLI_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($code === T_COMMENT) {
            ...
            $phpcsFile->addError($error, $stackPtr, 'StyleFunctionComment');
            return;
        } else if ($code !== T_DOC_COMMENT) {
            $error = 'Missing function doc comment';
            $phpcsFile->addError($error, $stackPtr, 'FunctionDocCommentRequired');
            return;
        }
        ...
        if ($prevToken !== $commentEnd) {
            $phpcsFile->addError('Missing function doc comment', $stackPtr, 'FunctionDocCommentRequired');
            return;
        }
        ...
        try {
            ...
            $phpcsFile->addError($e->getMessage(), $line, 'ParsingErrorFunctionComment');
            return;
        }
        ...
        if (is_null($comment) === true) {
            ...
            $phpcsFile->addError($error, $commentStart, 'NoEmptyFunctionDocComment');
            return;
        }
            ...
            switch ($content) {
                ...
                case 'param':
                    if (($con !== 'comment') and ($con !== 'param')) {
                        ...
                        $this->_currentFile->addError($error, ($commentStart + $tag), 'TagsOrderFunctionComment');
                    }
                    break;
        ...
        if (trim($short) === '') {
            ...
            $phpcsFile->addError($error, $commentStart, 'ShortDescRequiredFileComment');
            return;
        }
        ...
        if ($short !== '' and $newlineSpan > 0) {
            ...
            $phpcsFile->addError($error, ($commentStart + 1), 'NoExtraLineBeforeFunctionCommentShortDesc');
        }
        ...
        if (empty($long) === false) {
            ...
            if ($newlineBetween !== 2) {
                ...
                $phpcsFile->addError($error, ($commentStart + $newlineCount + 1), 'BlankLineBetweenDescriptionsFunctionComment');
            }
            ...
            if (preg_match('|[A-Z]|', $testLong[0]) === 0) {
                ...
                $phpcsFile->addError($error, ($commentStart + $newlineCount), 'LongDescFunctionCommentStartsWithCapitalLetter');
            }
        }
        ...
        if (count($params) > 1) {
            ...
            if ($newlineSpan !== 2) {
                ...
                $phpcsFile->addError($error, ($commentStart + $newlineCount), 'BlankLineRequiredBeforeTagsFunctionComment');
        ...
        if (preg_match('|[A-Z]|', $testShort[0]) === 0) {
            ...
            $phpcsFile->addError($error, ($commentStart + 1), 'ShortDescFunctionCommentStartsWithCapitalLetter');
        }
        ...
        foreach ($unknownTags as $errorTag) {
            ...
            $phpcsFile->addWarning($error, ($commentStart + $errorTag['line']), 'TagNotAllowedFunctionComment');
        }
    ...
    protected function _processSince($commentStart)
    {
        ...
        if ($since !== null) {
            ...
            if (count($index) > 1) {
                ...
                $this->_currentFile->addError($error, $errorPos, 'NoDuplicateTagFunctionComment');
                return;
            }
            ...
            if (empty($content) === true) {
                ...
                $this->_currentFile->addError($error, $errorPos, 'SinceTagMustContainVersionFunctionComment');
                return;
                ...
                if (preg_match('/^([0-9]+)\.([0-9]+)\.([0-9]+)/', $content) === 0) {
                    ...
                    $this->_currentFile->addError($error, $errorPos, 'VersionNumberSinceTagFunctionComment');
                }
            }
            ...
            if ($spacing !== 2) {
                ...
                $this->_currentFile->addError($error, $errorPos, 'TagIndentationFunctionComment');
            }
        }
        ...
        if (empty($sees) === false) {
            ...
            foreach ($sees as $i => $see) {
                ...
                if (empty($content) === true) {
                    ...
                    $this->_currentFile->addError($error, $errorPos, 'SeeTagNotEmptyFunctionComment');
                    continue;
                }
    ...
    protected function _processReturn($commentStart, $commentEnd)
    {
        ...
        if ($methodName === $className or $isSpecialMethod !== false) {
            ...
            if ($return !== null) {
                ...
                $this->_currentFile->addError($error, $errorPos, 'ReturnTagNotAllowedFunctionComment');
            }
        ...
        if ($return === null) {
            ...
            $this->_currentFile->addError($error, $commentEnd, 'TagRequiredFunctionComment');
            return;
        }
        ...
        if (empty($content) === true) {
            ...
            $this->_currentFile->addError($error, $errorPos, 'ReturnTypeRequiredReturnTagfunctionComment');
        } else {
            ...
            if ($content !== $suggestedType) {
                ...
                $this->_currentFile->addError($error, $errorPos, 'FunctionReturnType');
            }
                ...
                if (isset($tokens[$this->_functionToken]['scope_closer']) === true) {
                    ...
                    if ($return !== false) {
                        ...
                        if ($tokens[$semicolon]['code'] !== T_SEMICOLON) {
                            ...
                            $this->_currentFile->addError($error, $errorPos, 'FunctionCommentReturnTypeMatchReturnType');
                        }
                    }
                }
            } else {
                ...
                if (isset($tokens[$this->_functionToken]['scope_closer']) === true) {
                    ...
                    if ($return === false) {
                        ...
                        if ($tokens[$semicolon]['code'] === T_SEMICOLON) {
                            ...
                            $this->_currentFile->addError($error, $return, 'FunctionCommentReturnTypeMatchReturnType');
                        }
                    }
                }
            }
    ...
    protected function _processParams($commentStart, $commentEnd)
    {
        ...
        if (empty($params) === false) {
            ...
            if ((substr_count($params[(count($params) - 1)]->getWhitespaceAfter(),
                ...
                $this->_currentFile->addError($error, ($errorPos + 1), 'NoEmptyLineAfterLastParameterCommentFunctionComment');
            }
            ...
            if ($params[0]->getOrder() !== 2) {
                ...
                $this->_currentFile->addError($error, $errorPos, 'ParameterAppearAfterCommentFunctionComment');
            }
            ...
            foreach ($params as $param) {
                ...
                if (($isSpecialMethod === false) and ($param->getWhitespaceBeforeType() !== '  ')) {
                    ...
                    $this->_currentFile->addError($error, $errorPos, 'SpaceRequiredBeforeVariableTypeFunctionComment');
                }
                ...
                if ($previousParam !== null) {
                    ...
                    if ($param->alignsVariableWith($previousParam) === false) {
                        ...
                        $this->_currentFile->addError($error, $errorPos, 'VariableNamesParameterAlignedFunctionComment');
                    }

                    if ($param->alignsCommentWith($previousParam) === false) {
                        $error = 'The comments for parameters ' . $previousName . ' (' . ($pos - 1) . ') and '
                               . $paramName . ' (' . $pos . ') do not align';
                        $this->_currentFile->addError($error, $errorPos, 'CommentsParameterAlignedFunctionComment');
                    }
                }
                ...
                foreach ($typeNames as $typeName) {
                    ...
                    if ($typeName !== $suggestedName) {
                        ...
                        $this->_currentFile->addError($error, $errorPos, 'VariableTypeSupportedFunctionComment');
                        continue;
                    }
                    ...
                    if ($suggestedTypeHint !== '' and isset($realParams[($pos - 1)]) === true) {
                        ...
                        if ($typeHint === '') {
                            ...
                            $this->_currentFile->addError($error, ($commentEnd + 2), 'TypeHintRequiredFunctionComment');
                        } else if ($typeHint !== $suggestedTypeHint) {
                            $error = "Expected type hint \"$suggestedTypeHint\"; found \"$typeHint\""
                                   . " for $paramName at position $pos";
                            $this->_currentFile->addError($error, ($commentEnd + 2), 'TypeHintValidFunctionComment');
                        }
                ...
                if (isset($realParams[($pos - 1)]) === true) {
                    ...
                    if ($realName !== $param->getVarName()) {
                        ...
                        $this->_currentFile->addError($error, $errorPos, 'DocCommentMatchVariableName');
                    }
                } else {
                    // We must have an extra parameter comment
                    $error = 'Superfluous doc comment at position ' . $pos;
                    $this->_currentFile->addError($error, $errorPos, 'SuperfluousDocCommentFunctionComment');
                }

                if ($param->getVarName() === '') {
                    $error = 'Missing parameter name at position ' . $pos;
                     $this->_currentFile->addError($error, $errorPos, 'ParameterNameRequiredFunctionComment');
                }

                if ($param->getType() === '') {
                    $error = 'Missing type at position ' . $pos;
                    $this->_currentFile->addError($error, $errorPos, 'TypeRequiredFunctionComment');
                }

                if ($paramComment === '') {
                    $error = 'Missing comment for param "' . $paramName . '" at position ' . $pos;
                    $this->_currentFile->addError($error, $errorPos, 'ParameterCommentRequiredFunctionComment');
                } else {
                    ...
                    if ((preg_match('|[A-Z]|', $firstChar) === 0) and ($firstChar !== '(')) {
                        ...
                        $this->_currentFile->addError($error, $errorPos, 'ParameterCommentFunctionCommentStartsWithCapitalLetter');
                    }
                    ...
                    while ($nextParam !== false) {
                        ...
                        if (($nextCode === T_EQUAL) and ($paramName === $arg)) {
                            if (substr($paramComment, 0, 11) !== '(Optional) ') {
                                ...
                                $this->_currentFile->addError($error, $errorPos, 'OptionalParamCommentFunctionCommentStartsWithCapitalLetter');
            ...
            if ($spaceBeforeVar !== 1 and $spaceBeforeVar !== 10000 and $spaceBeforeComment !== 10000) {
                ...
                $this->_currentFile->addError($error, $longestType, 'SpaceRequiredAfterLongestTypeFunctionComment');
            }

            if ($spaceBeforeComment !== 1 and $spaceBeforeComment !== 10000) {
                $error = 'Expected 1 space after the longest variable name';
                $this->_currentFile->addError($error, $longestVar, 'SpaceRequiredAfterLongestVarFunctionComment');
            }
        }
        ...
        foreach ($diff as $neededParam) {
            ...
            $this->_currentFile->addError($error, $errorPos, 'ParameterDocCommentRequiredFunctionComment');
        }
    }
}
