...
/**
 * ZF_Sniffs_Commenting_VariableCommentSniff
 *
    ...
    public function processVar($commentStart, $commentEnd)
    {
        ...
        if ($var !== null) {
            ...
            if (count($index) > 1) {
                ...
                $this->currentFile->addError($error, $errorPos, 'NoDuplicateTagVariableComment');
                return;
            }

            if ($index[0] !== 1) {
                $error = 'The @var tag must be the first tag in a variable comment';
                $this->currentFile->addError($error, $errorPos, 'TagsOrderFunctionComment');
            }

            $content = $var->getContent();
            if (empty($content) === true) {
                $error = 'Var type missing for @var tag in variable comment';
                $this->currentFile->addError($error, $errorPos, 'VarTypeRequiredVariableComment');
                return;
            } else {
                ...
                if ($content !== $suggestedType) {
                    ...
                    $this->currentFile->addError($error, $errorPos, 'VarTypeValidVariableComment');
                }
            }
            ...
            if ($spacing !== $this->space) {
                ...
                $this->currentFile->addError($error, $errorPos, 'TagIndentationVariableComment');
            }
        } else {
            $error = 'Missing @var tag in variable comment';
            $this->currentFile->addError($error, $commentEnd, 'TagRequiredVariableComment');
        }
    ...
    public function processSince($commentStart, $commentEnd)
    {
        ...
        if ($since !== null) {
            ...
            if (empty($content) === true) {
                ...
                $this->currentFile->addError($error, $errorPos, 'SinceTagMustContainVersionVariableComment');
                return;
                ...
                if (preg_match('/^([0-9]+)\.([0-9]+)\.([0-9]+)/', $content) === 0) {
                    ...
                    $this->currentFile->addError($error, $errorPos, 'VersionNumberSinceTagVariableComment');
                }
            }
        ...
        if (empty($sees) === false) {
            foreach ($sees as $see) {
                ...
                if (empty($content) === true) {
                    ...
                    $this->currentFile->addError($error, $errorPos, 'SeeTagNotEmptyVariableComment');
                    continue;
                }
