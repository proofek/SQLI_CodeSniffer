...
/**
 * ZF_Sniffs_Commenting_ClassCommentSniff
 *
...
class ZF_Sniffs_Commenting_ClassCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($commentEnd !== false and $tokens[$commentEnd]['code'] === T_COMMENT) {
            $phpcsFile->addError("You must use \"/**\" style comments for a $type comment", $stackPtr, 'StyleClassComment');
            return;
        } else if ($commentEnd === false or $tokens[$commentEnd]['code'] !== T_DOC_COMMENT) {
            $phpcsFile->addError("Missing $type doc comment", $stackPtr, 'ClassDocCommentRequired');
            return;
        }
            ...
            if ($prevNonComment !== false) {
                ...
                if ($prevComment === false) {
                    ...
                    if ($newlineToken !== false) {
                        ...
                        if ($newlineToken !== false) {
                            ...
                            $phpcsFile->addError("Missing $type doc comment", ($stackPtr + 1), 'ClassDocCommentRequired');
                            return;
                        }
                    }
                }
                ...
                if ($prevTokenEnd !== false) {
                    ...
                    if ($blankLineBefore !== 2) {
                        ...
                        $phpcsFile->addError($error, ($commentStart - 1), 'BlankLineRequiredBeforeClassComment');
                    }
                }
            }
        }
        ...
        try {
            ...
            $phpcsFile->addError($e->getMessage(), $line, 'ParsingErrorClassComment');
            return;
        }
        ...
        if (is_null($comment) === true) {
            ...
            $phpcsFile->addError($error, $commentStart, 'NoEmptyClassDocComment');
            return;
        }
        ...
        if (trim($short) === '') {
            ...
            $phpcsFile->addError($error, $commentStart, 'ShortDescRequiredClassComment');
            return;
        }
        ...
        if ($short !== '' and $newlineSpan > 0) {
            ...
            $phpcsFile->addError($error, ($commentStart + 1), 'NoExtraLineBeforeClassCommentShortDesc');
        }
        ...
        if (empty($long) === false) {
            ...
            if ($newlineBetween !== 2) {
                ...
                $phpcsFile->addError($error, ($commentStart + $newlineCount + 1), 'BlankLineBetweenDescriptionsClassComment');
            }
            ...
            if (preg_match('|[A-Z]|', $testLong[0]) === 0) {
                ...
                $phpcsFile->addError($error, ($commentStart + $newlineCount), 'LongDescClassCommentStartsWithCapitalLetter');
            }
        }
        ...
        if (count($tags) > 1) {
            ...
            if ($newlineSpan !== 2) {
                ...
                $phpcsFile->addError($error, ($commentStart + $newlineCount), 'BlankLineRequiredBeforeTagsClassComment');
        ...
        if (substr_count($testShort, $phpcsFile->eolChar) !== 0) {
            ...
            $phpcsFile->addError($error, ($commentStart + 1), 'ShortDescClassCommentSingleLine');
        }

        if (preg_match('|[A-Z]|', $testShort[0]) === 0) {
            $error = ucfirst($type) . ' comment short description must start with a capital letter';
            $phpcsFile->addError($error, ($commentStart + 1), 'ShortDescClassCommentStartsWithCapitalLetter');
        }
        ...
        foreach ($unknownTags as $errorTag) {
            ...
            $phpcsFile->addWarning($error, ($commentStart + $errorTag['line']), 'TagNotAllowedClassComment');
            return;
        }
    ...
    protected function _processTags($commentStart, $commentEnd)
    {
        ...
        foreach ($foundTags as $tag => $info) {
            if ((array_key_exists($info, $tags) === false) and ($info !== 'comment')) {
                ...
                $this->currentFile->addError($error, ($commentStart + $tag), 'TagNotAllowedClassComment');
            }
        }
        ...
        foreach ($tags as $tag => $info) {
            ...
            if ($info['required'] === true and in_array($tag, $foundTags) === false) {
                ...
                $this->currentFile->addError($error, $commentEnd, 'TagRequiredClassComment');
                continue;
            }
                ...
                if ($info['allow_multiple'] === false) {
                    ...
                    foreach ($foundIndexes as $index) {
                        if ($index !== $count) {
                            ...
                            $this->currentFile->addError($error, $errorPosIndex, 'TagsMustBeGroupedClassComment');
                        }
            ...
            if ($foundIndexes[0] > $orderIndex) {
                ...
                $this->currentFile->addError($error, $errorPos, 'TagsOrderClassComment');
            }
        ...
        foreach ($indentation as $indentInfo) {
            if ($indentInfo['space'] !== 0 and $indentInfo['space'] !== ($longestTag + 1)) {
                ...
                $this->currentFile->addError($error, ($commentStart + $line), 'TagIndentationClassComment');
            }
        }
    ...
    protected function _processCategory($errorPos)
    {
        ...
        if ($category !== null) {
            ...
            if ($content !== 'Zend') {
                ...
                $this->currentFile->addError($error, $errorPos, 'CategorieNameClassComment');
            }
        }
        ...
        if ($package !== null) {
            ...
            if ($content !== '') {
                if (PHP_CodeSniffer::isUnderscoreName($content) !== true) {
                    ...
                    $this->currentFile->addError($error, $errorPos, 'PackageNameClassComment');
                }
            } else {
                $error = '@package tag must contain a name';
                $this->currentFile->addError($error, $errorPos, 'PackageTagMustContainNameClassComment');
            }
        }
        ...
        if ($subpackage !== null) {
            ...
            if ($content !== '') {
                if (PHP_CodeSniffer::isUnderscoreName($content) !== true) {
                    ...
                    $this->currentFile->addError($error, $errorPos, 'SubPackageNameClassComment');
                }
            } else {
                $error = '@subpackage tag must contain a name';
                $this->currentFile->addError($error, $errorPos, 'SubTagMustContainNameClassComment');
            }
        }
            ...
            foreach ($sees as $see) {
                ...
                if ($content !== '') {
                    if (PHP_CodeSniffer::isUnderscoreName($content) !== true) {
                        ...
                        $this->currentFile->addError($error, $errorPos, 'SeeNameClassComment');
                    }
                } else {
                    $error = '@see tag must contain a name';
                    $this->currentFile->addError($error, $errorPos, 'SeeTagMustContainNameClassComment');
                }
            }
        }
        ...
        if ($since !== null) {
            ...
            if ($content !== '') {
                if (preg_match('/^([0-9]+)\.([0-9]+)\.([0-9]+)/', $content) === 0) {
                    ...
                    $this->currentFile->addError($error, $errorPos, 'VersionNumberSinceTagClassComment');
                }
            } else {
                $error = '@since tag must contain a name';
                $this->currentFile->addError($error, $errorPos, 'SinceTagMustContainNameClassComment');
            }
        }
    ...
    protected function _processLicense($errorPos)
    {
        ...
        if ($license !== null) {
            ...
            if ($value !== 'http://framework.zend.com/license/new-bsd' and $comment !== 'New BSD License') {
                ...
                $this->currentFile->addError($error, $errorPos, 'LicenceTagClassComment');
            }
        }
    }
