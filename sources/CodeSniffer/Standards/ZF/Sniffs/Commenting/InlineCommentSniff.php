 ...
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class ZF_Sniffs_Commenting_InlineCommentSniff implements SQLI_CodeSniffer_Sniff
{
    /**
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === T_DOC_COMMENT) {
            ...
            if (in_array($tokens[$nextToken]['code'], $ignore) === true) {
                ...
                if (substr($tokens[$stackPtr]['content'], 0, 3) === '/**') {
                    ...
                    $phpcsFile->addError($error, $stackPtr, 'NoInlineDocBlockComment');
                }
            }
        }
        ...
        if ($tokens[$stackPtr]['content']{0} === '#') {
            ...
            $phpcsFile->addError($error, $stackPtr, 'NoPearStyleComment');
        }
        ...
        if ($spaceCount === 0) {
            $error = 'No space before comment text; expected "// ' . substr($comment, 2)
                   . '" but found "' . $comment . '"';
            $phpcsFile->addError($error, $stackPtr, 'NoSpaceBeforeCommentText');
        }

        if ($spaceCount > 1) {
            $error = $spaceCount . ' spaces found before inline comment; expected "// '
                   . substr($comment, (2 + $spaceCount)) . '" but found "' . $comment . '"';
            $phpcsFile->addError($error, $stackPtr, 'NoSpaceBeforeInlineComment');
        }
        ...
        if ($commentText === '') {
            ...
            $phpcsFile->addError($error, $stackPtr, 'NoBlockComment');
            return;
        }

        if (preg_match('|[A-Z]|', $commentText[0]) === 0) {
            $error = 'Inline comments must start with a capital letter';
            $phpcsFile->addError($error, $topComment, 'InlineCommentsStartsWithCapitalLetter');
        }
        ...
        $phpcsFile->addError($error, $stackPtr, 'NoBlankLineFollowingInlineComment');
    }
}
