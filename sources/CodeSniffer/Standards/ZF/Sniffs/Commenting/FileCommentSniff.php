 ...
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class ZF_Sniffs_Commenting_FileCommentSniff implements SQLI_CodeSniffer_Sniff
{
    /**
        ...
        if ($tokens[$commentStart]['code'] === T_CLOSE_TAG) {
            ...
            if ($tokens[$commentStart]['line'] !== 2) {
                $phpcsFile->addError('File doc comment must be in the second line, no whitespace allowed',
                                     ($stackPtr + 1), 'FileDocCommentInSecondLine');
                return;
            }
        ...
        if ($package !== null) {
            ...
            if ($content !== '') {
                if (PHP_CodeSniffer::isUnderscoreName($content) !== true) {
                    ...
                    $this->_currentFile->addError($error, $errorPos, 'PackageNameFileComment');
                }
            } else {
                $error = '@package tag must contain a name';
                $this->_currentFile->addError($error, $errorPos, 'PackageTagMustContainNameFileComment');
            }
        }
