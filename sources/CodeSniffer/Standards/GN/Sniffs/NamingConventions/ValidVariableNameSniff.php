...
/**
 * GN_Sniffs_NamingConventions_ValidVariableNameSniff
 ...
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class GN_Sniffs_NamingConventions_ValidVariableNameSniff extends PHP_CodeSniffer_Standards_AbstractVariableSniff implements SQLI_CodeSniffer_Sniff
{
    /**
...
// SQLI : les premier caractère ne doit pas être un underscore
		        if (substr($objVarName, 0, 1) === '_') {
				    $phpcsFile->addEvent(
				        'NO_START_UNDERSCORE_VALID_VARIABLE', 
				        array('varname' => $objVarName),
				        $stackPtr
				    );        	
   		            return;
		        }
...
//                }
				    ...
				    $phpcsFile->addEvent(
				        'NOT_VALID_CAMEL_VALID_VARIABLE', 
				        array('varname' => $originalVarName),
				        $var
				    );        	
                } else if (preg_match('|\d|', $objVarName)) {
				    $phpcsFile->addEvent(
				        'CONTAINS_NUMBER_VALID_VARIABLE', 
				        array('varname' => $originalVarName),
				        $stackPtr
				    );        	
                }
...
//        }
        ...
        if (PHP_CodeSniffer::isCamelCaps($varName, false, true, false) === false) {
		    $phpcsFile->addEvent(
		        'ATTR_NOT_VALID_CAMEL_VALID_VARIABLE', 
		        array('varname' => $varName),
		        $stackPtr
		    );        	
        } else if (preg_match('|\d|', $varName)) {
