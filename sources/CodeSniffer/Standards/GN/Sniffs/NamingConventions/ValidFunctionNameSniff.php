<?php
/**
 * GN_Sniffs_NamingConventions_ValidFunctionNameSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class GN_Sniffs_NamingConventions_ValidFunctionNameSniff extends PHP_CodeSniffer_Standards_AbstractScopeSniff implements SQLI_CodeSniffer_Sniff
{
    ...
    protected function processTokenWithinScope(PHP_CodeSniffer_File $phpcsFile, $stackPtr, $currScope)
    {
        ...
        if (preg_match('|^__|', $methodName) !== 0) {
            ...
            if (in_array($magicPart, $this->_magicMethods) === false) {
			     $phpcsFile->addEvent(
			        'NAME_INVALID_MAGIC_METHOD_VALID_FUNCTION', 
			        array('methodname' => "$className::$methodName"),
			        $stackPtr
			     );        	
            }
...
//        }
        ...
        if (PHP_CodeSniffer::isCamelCaps($testMethodName, false, $isPublic, false) === false) {
            if ($scopeSpecified === true) {
			    $phpcsFile->addEvent(
			        'SCOPE_METHOD_NAME_NOT_CAMEL_VALID_FUNCTION', 
			        array('scope' => ucfirst($scope), 'methodname' => "$className::$methodName"),
			        $stackPtr
			    );        	
            } else {
			    $phpcsFile->addEvent(
			        'METHOD_NAME_NOT_CAMEL_VALID_FUNCTION', 
			        array('methodname' => "$className::$methodName"),
			        $stackPtr
			    );        	
            }
    ...
    protected function processTokenOutsideScope(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        //if ($packagePart !== '') {
            if ($functionName{0} === '_') {
			    $phpcsFile->addEvent(
			        'NAME_INVALID_PRIVATE_METHOD_VALID_FUNCTION', 
			        array('name' => $functionName),
			        $stackPtr
			    );        	
                return;
        ...
        if (trim($camelCapsPart) === '') {
			$phpcsFile->addEvent(
			   'NAME_NOT_VALID_INCOMPLETE_VALID_FUNCTION', 
			   array('name' => $functionName),
			   $stackPtr
			);        	
            return;
        ...
        if ($validName === false) {
			...
			$phpcsFile->addEvent(
			   'NAME_INVALID_CONSIDER_NEWNAME_VALID_FUNCTION', 
			   array('name' => $functionName, 'newname' => $newName),
			   $stackPtr
			);        	
        }
